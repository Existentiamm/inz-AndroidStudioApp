package com.example.myapplication;

public class DataPets {

    public static String[] names = new String[]{
            "Kot1", "Kot2", "Kot3", "Kot4"
//na razie na sztywno, potem zmodyfikować
    };

    public static int[] images = new int[]{
R.drawable.cat1, R.drawable.cat2, R.drawable.cat3, R.drawable.toska
//na razie na sztywno, potem zmodyfikować
    };

    public static String[] meds = new String[]{
            "meds1", "meds2", "meds3", "meds4", "meds5", "meds6", "meds7"
    };


    public static String[] treatment  = new String[]{
            "treatment1", "treatment2", "treatment3", "treatment4"
    };

    public static String[] diseases = new String[]{
            "diseases1", "diseases2", "diseases3", "diseases4"
    };

    public static String[] dodatkowe_informaje = new String[]{
            "dodatkowe informacje", "dodatkowe informacje", "dodatkowe informacje", "dodatkowe informacje", "dodatkowe informacje", "dodatkowe informacje", "dodatkowe informacje"
    };

}
